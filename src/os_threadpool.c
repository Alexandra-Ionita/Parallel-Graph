...
static void *thread_loop_function(void *arg)
{
	...
	while (1) {
		...
		pthread_mutex_lock(&tp->lockMutex);

		t = dequeue_task(tp);
		if (t == NULL) {
			pthread_mutex_unlock(&tp->lockMutex);
			break;
		}
		t->action(t->argument);
		destroy_task(t);
...
void wait_for_completion(os_threadpool_t *tp)
{
	...
	pthread_cond_wait(&tp->cond, &tp->lockMutex);
	pthread_mutex_unlock(&tp->lockMutex);
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	tp->tasks = NULL;
	pthread_mutex_init(&tp->lockMutex, NULL);
...
void destroy_threadpool(os_threadpool_t *tp)
{
	...
	pthread_mutex_destroy(&tp->lockMutex);
